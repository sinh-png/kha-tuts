...
var Player = function() {
	this.position = new kha_math_FastVector2(384.,568);
};
...
Player.prototype = {
	...
	,render: function(g) {
		g.set_color(-65536);
		g.drawRect(this.position.x,this.position.y,32,32,5);
	}
	,__class__: Player
};
...
Ball.prototype = {
	...
	,render: function(g) {
		...
		kha_graphics2_GraphicsExtension.drawCircle(g,this.position.x,this.position.y,this.radius,5);
	}
	,update: function() {
		...
		if(this.velocity.x > 0 && this.position.x > 800 - this.radius || this.velocity.x < 0 && this.position.x < this.radius) {
			this.velocity.x *= -1;
		}
		if(this.velocity.y > 0 && this.position.y > 600 - this.radius) {
			this.velocity.y = -this.radius / 5;
		}
	}
	,spawn: function(red) {
		this.radius = 20 + 20 * Math.random();
		this.position.x = this.radius + (800 - this.radius * 2) * Math.random();
		this.position.y = -this.radius;
		this.velocity.x = -3 + 6 * Math.random();
		this.velocity.y = 0;
		this.color = red?-65536:-16711681;
		this.active = true;
	}
	,overlapPlayer: function() {
		var cx = Math.min(Math.max(Main.player.position.x,this.position.x),Main.player.position.x + 32);
		var cy = Math.min(Math.max(Main.player.position.y,this.position.y),Main.player.position.y + 32);
		var dx = this.position.x - cx;
		var dy = this.position.y - cy;
...
Main.main = function() {
	kha_System.init({ title : "Ball Avoider", width : 800, height : 600},function() {
		kha_Assets.loadEverything(Main.onAssetsLoaded);
	});
};
Main.startGame = function() {
	Main.score = 0;
	Main.redBall.spawn(true);
	Main._playing = true;
};
Main.scoreUp = function() {
	Main.score++;
	if(_$UInt_UInt_$Impl_$.gt(Main.score,Main.hiscore)) {
...
Main.spawnBlueBall = function() {
	...
	var ball1 = new Ball();
	ball1.spawn(false);
	Main.blueBalls.push(ball1);
};
Main.onAssetsLoaded = function() {
	kha_System.notifyOnRender(Main.onRender);
	kha_Scheduler.addTimeTask(Main.onUpdate,0,0.016666666666666666);
	kha_input_Mouse.get().notify(Main.onMouseDown,null,Main.onMouseMove,null);
	Main.backbuffer = kha_Image.createRenderTarget(800,600);
	Main._pipeline = new kha_graphics4_PipelineState();
	Main._pipeline.inputLayout = [new kha_graphics4_VertexStructure()];
	Main._pipeline.vertexShader = kha_Shaders.painter_colored_vert;
	Main._pipeline.fragmentShader = kha_Shaders.grid_frag;
	Main._pipeline.compile();
	Main._uTimeLoc = Main._pipeline.getConstantLocation("time");
	Main._uPlayerXLoc = Main._pipeline.getConstantLocation("playerX");
};
Main.onRender = function(framebuffer) {
	var g2 = Main.backbuffer.get_g2();
	var g4 = Main.backbuffer.get_g4();
	g2.begin();
	g2.set_pipeline(Main._pipeline);
	g4.setFloat(Main._uTimeLoc,kha_System.get_time());
	g4.setFloat(Main._uPlayerXLoc,Main.player.position.x + 16.);
	g2.fillRect(0,0,800,600);
	g2.set_pipeline(null);
	g2.set_font(kha_Assets.fonts.ClearSans_Regular);
	if(Main._playing) {
		var _g = 0;
		var _g1 = Main.blueBalls;
		while(_g < _g1.length) {
		...
		Main.redBall.render(g2);
		Main.player.render(g2);
	} else {
		g2.set_color(-1);
		g2.set_fontSize(40);
		g2.drawString("COLLECT RED BALL - AVOID BLUE BALLS",(800 - g2.get_font().width(g2.get_fontSize(),"COLLECT RED BALL - AVOID BLUE BALLS")) / 2,300. - g2.get_font().height(g2.get_fontSize()) - 4);
		g2.drawString("CLICK TO PLAY",(800 - g2.get_font().width(g2.get_fontSize(),"CLICK TO PLAY")) / 2,308.);
	}
	var str = "SCORE: " + Std.string(_$UInt_UInt_$Impl_$.toFloat(Main.score)) + "                HI-SCORE: " + Std.string(_$UInt_UInt_$Impl_$.toFloat(Main.hiscore));
	g2.set_color(-1);
	g2.set_fontSize(40);
	g2.drawString(str,(800 - g2.get_font().width(g2.get_fontSize(),str)) / 2,5);
	g2.end();
	g2 = framebuffer.get_g2();
	g2.begin();
	var scaleX = framebuffer.get_width() / 800;
	var scaleY = framebuffer.get_height() / 600;
	...
	g2.set_color(-1);
	g2.drawImage(Main.backbuffer,(framebuffer.get_width() - 800 * scale) / 2,(framebuffer.get_height() - 600 * scale) / 2);
	g2.setTransformation(null);
	g2.transformations[g2.transformations.length - 1] = null;
	g2.end();
};
...
Main.onMouseDown = function(button,mouseX,mouseY) {
	if(!Main._playing) {
		Main.startGame();
	}
};
Main.onMouseMove = function(mouseX,mouseY,_,_1) {
	if(!Main._playing) {
		return;
	}
	Main.player.position.x = mouseX - 16.;
	if(Main.player.position.x < 0) {
		Main.player.position.x = 0;
	} else if(Main.player.position.x > 768) {
		Main.player.position.x = 768;
	}
};
Math.__name__ = true;
...
var kha_FontList = function() {
	this.ClearSans_RegularDescription = { files : ["ClearSans_Regular.ttf"], type : "font", name : "ClearSans_Regular"};
	this.ClearSans_RegularName = "ClearSans_Regular";
	this.ClearSans_Regular = null;
};
$hxClasses["kha.FontList"] = kha_FontList;
kha_FontList.__name__ = true;
kha_FontList.prototype = {
	ClearSans_Regular: null
	,ClearSans_RegularName: null
	,ClearSans_RegularDescription: null
	,ClearSans_RegularLoad: function(done) {
		kha_Assets.loadFont("ClearSans_Regular",function(font) {
			done();
		});
	}
	,ClearSans_RegularUnload: function() {
		this.ClearSans_Regular.unload();
		this.ClearSans_Regular = null;
	}
...
kha_Shaders.init = function() {
	var data = Reflect.field(kha_Shaders,"grid_fragData");
	var bytes = haxe_Unserializer.run(data);
	kha_Shaders.grid_frag = new kha_graphics4_FragmentShader(kha_internal_BytesBlob.fromBytes(bytes));
	var data1 = Reflect.field(kha_Shaders,"painter_colored_fragData");
	var bytes1 = haxe_Unserializer.run(data1);
	kha_Shaders.painter_colored_frag = new kha_graphics4_FragmentShader(kha_internal_BytesBlob.fromBytes(bytes1));
	var data2 = Reflect.field(kha_Shaders,"painter_colored_vertData");
	var bytes2 = haxe_Unserializer.run(data2);
	kha_Shaders.painter_colored_vert = new kha_graphics4_VertexShader(kha_internal_BytesBlob.fromBytes(bytes2));
	var data3 = Reflect.field(kha_Shaders,"painter_image_fragData");
	var bytes3 = haxe_Unserializer.run(data3);
	kha_Shaders.painter_image_frag = new kha_graphics4_FragmentShader(kha_internal_BytesBlob.fromBytes(bytes3));
	var data4 = Reflect.field(kha_Shaders,"painter_image_vertData");
	var bytes4 = haxe_Unserializer.run(data4);
	kha_Shaders.painter_image_vert = new kha_graphics4_VertexShader(kha_internal_BytesBlob.fromBytes(bytes4));
	var data5 = Reflect.field(kha_Shaders,"painter_text_fragData");
	var bytes5 = haxe_Unserializer.run(data5);
	kha_Shaders.painter_text_frag = new kha_graphics4_FragmentShader(kha_internal_BytesBlob.fromBytes(bytes5));
	var data6 = Reflect.field(kha_Shaders,"painter_text_vertData");
	var bytes6 = haxe_Unserializer.run(data6);
	kha_Shaders.painter_text_vert = new kha_graphics4_VertexShader(kha_internal_BytesBlob.fromBytes(bytes6));
	var data7 = Reflect.field(kha_Shaders,"painter_video_fragData");
	var bytes7 = haxe_Unserializer.run(data7);
	kha_Shaders.painter_video_frag = new kha_graphics4_FragmentShader(kha_internal_BytesBlob.fromBytes(bytes7));
	var data8 = Reflect.field(kha_Shaders,"painter_video_vertData");
	var bytes8 = haxe_Unserializer.run(data8);
	kha_Shaders.painter_video_vert = new kha_graphics4_VertexShader(kha_internal_BytesBlob.fromBytes(bytes8));
};
...
kha_Scheduler.lastNow = 0;
kha_Shaders.grid_fragData = "s1139:";
